...
export const debouncedRef = <T>(startValue: T, wait = 300): Ref<T> => {
  let state = startValue; // internal value
  let timer: ReturnType<typeof setTimeout> | null = null;
 
  return customRef<T>((track, trigger) => ({
    get: () => {
      track();       // tracks dependency, like normal ref
      return state;  // return current state
    },
    set: (newVal: T) => {
      if (timer) clearTimeout(timer); // cancel previous debounce timer

      timer = setTimeout(() => {
        state = newVal; // update internal value
        trigger();      // notify Vue reactivity system
      }, wait);
    },
  }));
};
